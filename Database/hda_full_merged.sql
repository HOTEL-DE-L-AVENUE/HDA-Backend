-- ============================================================================
-- HDA — Script SQL consolidé (schéma complet + données)
-- Généré automatiquement en fusionnant :
--   hda.sql (dump principal, schéma + données au 2026-07-16)
--   + alchool.sql (module Alcool)
--   + bar_complete.sql (module Bar & Lounge, version optimisée)
--   + ajustements runtime des modèles Node (colonnes auto-ajoutées par
--     restaurantModel.js / barOrder.model.js / barTransaction.model.js)
--   + fix_accents.sql (catalogue bar corrigé, accents + prix Ariary)
--   + tous les fichiers de Database/migrations/ (schéma additif, dans
--     l'ordre de dépendance correct)
--
-- Fichiers volontairement EXCLUS de cette fusion (voir le résumé fourni
-- avec ce script pour le détail des raisons) :
--   - hda_schema_clean.sql / hda_schema_only.sql (dérivés redondants de hda.sql)
--   - bar_lounge.sql (version obsolète, remplacée par bar_complete.sql)
--   - casino_reference_data.sql (données déjà présentes dans hda.sql)
--   - migrations/reset_accommodation_data.sql (script de purge opérationnelle,
--     pas une migration de schéma — supprimerait les données qu'on vient d'insérer)
--
-- À exécuter une seule fois sur une base vide.
-- ============================================================================

-- ============================================================================
-- SECTION 1 : SCHEMA + DONNEES PRINCIPALES (hda.sql, dump du 2026-07-16)
-- ============================================================================
...
-- ============================================================================
-- SECTION 2 : MODULE ALCOOL (alchool.sql)
-- ============================================================================
...
-- ============================================================================
-- SECTION 3 : MODULE BAR & LOUNGE (bar_complete.sql, version optimisee)
-- ============================================================================
...
(1, 'Caisse 1', 'ACTIF'),
(2, 'Caisse 2', 'ACTIF');-- --------------------------------------------------------------------------
-- Complément module Bar : tables `bar_orders` / `bar_order_items` et colonne
-- `bar_transactions.order_id`, créées dynamiquement au démarrage par
-- barOrder.model.js / barTransaction.model.js (ensureBarOrderTables /
-- ensureBarTransactionsSchema). On les fige ici pour que le schéma soit
-- complet dès l'import, sans dépendre de cette auto-migration au runtime.
-- --------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `bar_orders` (
  ...
  `client_name` varchar(150) DEFAULT NULL,
  `table_id` bigint(20) UNSIGNED DEFAULT NULL,
  `nombre_personnes` int(11) UNSIGNED NOT NULL DEFAULT 1,
...
ALTER TABLE `bar_transactions`
  ADD COLUMN `order_id` BIGINT(20) UNSIGNED NULL AFTER `table_id`;

-- ============================================================================
-- SECTION 3b : Correction du catalogue bar - accents UTF-8 et prix Ariary (fix_accents.sql)
-- ============================================================================
...
-- ============================================================================
-- SECTION 4 : MIGRATIONS CASINO (joueurs, sieges, KYC, cash/rack check, role croupier)
-- ============================================================================
...
ALTER TABLE casino_tables_jeu
    ADD COLUMN type_partie ENUM('JEU_SIMPLE','TOURNOI') NOT NULL DEFAULT 'JEU_SIMPLE' AFTER type_jeu,
    ADD COLUMN nombre_places INT UNSIGNED NOT NULL DEFAULT 8 AFTER type_partie,
    ADD COLUMN duree_jeu_simple_minutes INT UNSIGNED NOT NULL DEFAULT 120 AFTER nombre_places;
...
-- ============================================================================
-- SECTION 5 : MIGRATIONS HEBERGEMENT / HOTEL
-- ============================================================================
...
ALTER TABLE reservations
  ADD COLUMN montant_brut BIGINT NULL,
  ADD COLUMN remise_pourcentage DECIMAL(5,2) NOT NULL DEFAULT 0,
  ADD COLUMN montant_remise BIGINT NOT NULL DEFAULT 0,
  ADD COLUMN remise_validee_par BIGINT NULL,
  ADD COLUMN remise_validee_at DATETIME NULL;

ALTER TABLE room_types
  ADD COLUMN prix_base BIGINT NOT NULL DEFAULT 0;

ALTER TABLE room_equipments
  ADD COLUMN zone ENUM('CHAMBRE','SALLE_DE_BAIN') NOT NULL DEFAULT 'CHAMBRE';
...
ALTER TABLE stocks
  ADD COLUMN seuil_minimum DECIMAL(15,2) NOT NULL DEFAULT 0,
  ADD COLUMN etat ENUM('DISPONIBLE','EN_LAVAGE','USE','ENDOMMAGE','REBUT','PERDU') NOT NULL DEFAULT 'DISPONIBLE';
...
ALTER TABLE room_maintenance
  ...
  ADD COLUMN equipment_label VARCHAR(255) NULL,
  ADD COLUMN worker_id BIGINT UNSIGNED NULL,
  ADD COLUMN execution_date DATE NULL,
  ADD COLUMN finish_date DATE NULL,
  ADD COLUMN materials_cost BIGINT NOT NULL DEFAULT 0,
  ADD COLUMN labor_cost BIGINT NOT NULL DEFAULT 0,
...
-- ============================================================================
-- SECTION 6 : MIGRATIONS RESTAURANT / USERS
-- ============================================================================
...
ALTER TABLE `products`
ADD COLUMN `subcategory_id` bigint(20) UNSIGNED NULL AFTER `category_id`;
...
ALTER TABLE `products`
ADD COLUMN `portion_size` decimal(10,2) NULL DEFAULT NULL COMMENT 'Standard portion size for the product',
ADD COLUMN `portion_unite` varchar(20) NULL DEFAULT NULL COMMENT 'Unit for portion size (g, ml, etc.)';
...
-- --------------------------------------------------------------------------
-- Colonnes ajoutées dynamiquement au runtime par restaurantModel.js
-- (ensureRestaurantSchema) sur des installations plus anciennes. Figées ici
-- pour que le schéma soit complet dès l'import. Doit précéder
-- add_restaurant_order_customizations.sql, qui insère `notes` juste après
-- `cloture_at`.
-- --------------------------------------------------------------------------

ALTER TABLE `orders` ADD COLUMN `table_id` BIGINT UNSIGNED NULL AFTER `client_id`;
ALTER TABLE `orders` ADD COLUMN `cloturee` TINYINT(1) NOT NULL DEFAULT 0 AFTER `statut`;
ALTER TABLE `orders` ADD COLUMN `cloture_at` DATETIME NULL AFTER `cloturee`;

ALTER TABLE `payments` ADD COLUMN `order_id` BIGINT UNSIGNED NULL AFTER `id`;
ALTER TABLE `payments` ADD COLUMN `date_paiement` DATETIME NULL AFTER `moyen_paiement`;

ALTER TABLE `financial_transactions` ADD COLUMN `cloturee` TINYINT(1) NOT NULL DEFAULT 0 AFTER `statut_sync`;
ALTER TABLE `financial_transactions` ADD COLUMN `cloture_at` DATETIME NULL AFTER `cloturee`;
